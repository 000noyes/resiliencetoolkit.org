 ...
 *
 * It also writes functions/lib/page-hashes.generated.js: one content hash per
 * built page, keyed by route, for EVERY page in dist (precache exclusions
 * included). The root middleware sets an ETag on each HTML page from this map
 * and answers 304 to a matching If-None-Match, which is what lets the worker's
 * saved-copy check and the precache refill cost about 1 KB for an unchanged
 * page. The file is generated, gitignored, and typed by the tracked
 * page-hashes.generated.d.ts next to it, so a checkout with no build still
 * type-checks. Cloudflare bundles functions/ after the build command, so the
 * file is always present when the middleware is compiled.
 *
...
const pageHashesDir = join(rootDir, 'functions', 'lib');
const pageHashesPath = join(pageHashesDir, 'page-hashes.generated.js');
...
// --- Write the page hash map for the middleware ---

const sortedHashes = Object.fromEntries(
  Object.keys(pageHashes).sort().map((route) => [route, pageHashes[route]])
);
mkdirSync(pageHashesDir, { recursive: true });
writeFileSync(
  pageHashesPath,
  '// Generated by scripts/generate-sw-precache.mjs at postbuild. Do not edit.\n' +
    `export const PAGE_HASHES = ${JSON.stringify(sortedHashes, null, 2)};\n`,
  'utf-8'
);
