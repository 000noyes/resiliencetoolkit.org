 ...
 *
 * One place to answer "can this browser durably hold your work right now?" It
 * lifts the per-component IndexedDB/quota detection that used to live inside
 * DataTable into a single module the app-wide StorageHealthBanner and any
 * editor can read. The durability UI is driven off the persisted() grant
 * boolean and the acute unavailable/full states — never raw estimate() quota
 * numbers (browsers deliberately fuzz those).
 *
...
const UNAVAILABLE_MESSAGE =
  'Your browser is not saving your work on this device right now. This can happen in private or restricted browsing. Anything you enter will be lost when you close the page, so use Export to keep a copy.';

const FULL_MESSAGE =
  'This device is out of storage space. You can still export what you already have, but new entries will not save until you free up space.';

const AT_RISK_MESSAGE =
  'Your work is saved on this device and stays private, but your browser can clear it to free up space. Back it up with Export so you never lose it.';
